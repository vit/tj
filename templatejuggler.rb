...
class TemplateJuggler
	...
	class Scope
		# Сюда шаблон может записать (для LM) id шаблона, который должен быть обработан следующим
		# (если не годится стандартный порядок -- с перемещением по каталогам снизу вверх
		# и использованием шаблонов autohandler)
		# !!! LM пока не умеет обрабатывать относительный путь.
		...
		# Сюда LM записывает id шаблона, который обрабатывается в настоящий момент
	...
	# Объект, имеющий метод get и возвращающий по заданному id.
	# Для самой LM не важно, где именно хранятся шаблоны. 
